...
def robot_dynamics_3state(st, v_cmd, delta_cmd):
    dx = v_cmd * np.cos(st[2])
    dy = v_cmd * np.sin(st[2])
    dpsi = (v_cmd / L) * np.tan(delta_cmd)
    return np.array([dx, dy, dpsi], dtype=float)


def rk4_step_3state(state, v_cmd, delta_cmd):
    k1 = robot_dynamics_3state(state, v_cmd, delta_cmd)
    k2 = robot_dynamics_3state(state + Ts / 2 * k1, v_cmd, delta_cmd)
    k3 = robot_dynamics_3state(state + Ts / 2 * k2, v_cmd, delta_cmd)
    k4 = robot_dynamics_3state(state + Ts * k3, v_cmd, delta_cmd)

    new_state = state + (Ts / 6) * (k1 + 2 * k2 + 2 * k3 + k4)
    new_state[2] = wrap_angle(new_state[2])
    return new_state


def get_robot_xy_velocity(robot):
    v_now = robot.last_v_cmd
    psi_now = robot.state[2]
    return np.array([
        v_now * np.cos(psi_now),
        v_now * np.sin(psi_now)
    ], dtype=float)
...
class Robot:
    def __init__(self, id_num, agent_id, trajectory_fn, color, label):
        ...
        self.state = np.array([(start_ref[0]), float(start_ref[1]), float(start_ref[2])], dtype=float)
...
def create_cbf_qp_solver():
    ...
    delta_star = opti_qp.variable()
    slack = opti_qp.variable(MAX_OTHER_ROBOTS)

    delta_nom_in = opti_qp.parameter()
    X_curr = opti_qp.parameter(3)          # [x, y, psi]
    v_curr = opti_qp.parameter()           # referenshastighet som används detta steg
    obs_pos_qp = opti_qp.parameter(2, MAX_OTHER_ROBOTS)
    obs_vel_qp = opti_qp.parameter(2, MAX_OTHER_ROBOTS)

    state_sym = ca.SX.sym('x_qp', 3)
    delta_sym = ca.SX.sym('delta_qp')
    v_sym = ca.SX.sym('v_qp')

    f_kin = ca.Function(
        'f_kin_qp',
        [state_sym, v_sym, delta_sym],
        [ca.vertcat(
            v_sym * ca.cos(state_sym[2]),
            v_sym * ca.sin(state_sym[2]),
            (v_sym / L) * ca.tan(delta_sym)
        )]
    )

    k1_c = f_kin(X_curr, v_curr, delta_star)
    k2_c = f_kin(X_curr + Ts / 2 * k1_c, v_curr, delta_star)
    k3_c = f_kin(X_curr + Ts / 2 * k2_c, v_curr, delta_star)
    k4_c = f_kin(X_curr + Ts * k3_c, v_curr, delta_star)
    X_next_c = X_curr + Ts / 6 * (k1_c + 2 * k2_c + 2 * k3_c + k4_c)

    cost_qp = 5.0 * (delta_star - delta_nom_in) ** 2
    ...
    opti_qp.subject_to(opti_qp.bounded(-delta_max, delta_star, delta_max))

    opti_qp.minimize(cost_qp)
    opti_qp.solver('ipopt', {'print_time': 0, 'ipopt.print_level': 0, 'ipopt.sb': 'yes'})

    return opti_qp, delta_star, delta_nom_in, X_curr, v_curr, obs_pos_qp, obs_vel_qp, slack
...
def update(frame):
    ...
    for r in robots:
        ...
        if not r.finished:
            ...
            delta_lqr = np.clip(deltar + delta_delta, -delta_max, delta_max)

            obs_pos_data, obs_vel_data = build_other_robot_data(r)

            opti_qp, delta_star_v, delta_nom_p, X_curr_p, v_curr_p, obs_pos_p, obs_vel_p, slack_v = qp_solvers[r.id]
            opti_qp.set_value(delta_nom_p, float(delta_lqr))
            opti_qp.set_value(X_curr_p, r.state)
            opti_qp.set_value(v_curr_p, float(vr))
            opti_qp.set_value(obs_pos_p, obs_pos_data)
            opti_qp.set_value(obs_vel_p, obs_vel_data)

            try:
                sol_qp = opti_qp.solve()
                delta_cmd = float(sol_qp.value(delta_star_v))
            except RuntimeError:
                delta_cmd = float(delta_lqr)

            pos_error = np.hypot(x - xr, y - yr)
            v_cmd = float(np.clip(vr - 0.4 * pos_error, 0.0, v_max))

            r.last_delta_cmd = delta_cmd
            r.last_v_cmd = v_cmd

            r.state = rk4_step_3state(r.state, v_cmd, delta_cmd)
            ...
            r.state_hist.append([r.state[0], r.state[1], r.state[2], v_cmd])
            r.control_hist.append([delta_cmd])
...
ani.save('lqr_qp_cbf_no_accel.mp4', writer='ffmpeg', fps=30, dpi=100)
plt.show()
